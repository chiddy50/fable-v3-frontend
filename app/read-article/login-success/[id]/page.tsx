...
const ReadArticleLoginSuccessPage = ({params: {id}}: Props) => {
    ...
    const authenticate = async () => {
        try {
            ...
            const articleId = sessionStorage.getItem("articleId") ?? localStorage.getItem("articleId");

            setArticleId(articleId ?? "");
            setTimeout(() => {
                push(`/read-article/${articleId}`)
            }, 4000);
            ...
            >
                {
                    ...
                    <div className='w-[85%] sm:w-[60%] md:w-[40%] lg:w-[30%] border p-5 rounded-2xl shadow-xl'>                    
                        ...
                        <div className="mt-5">
                            <Link href={`/read-article/${articleId}`} className='w-full'>
                                <Button size="sm" className='text-xs w-full'>Read Article</Button>
                            </Link>
                        </div>
                    </div>
                }
...
export default ReadArticleLoginSuccessPage
