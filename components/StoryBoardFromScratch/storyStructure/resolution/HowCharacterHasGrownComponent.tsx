...
interface HowCharacterHasGrownComponentProps {
  openHowCharacterHasGrownComponentModal: boolean;
  setOpenHowCharacterHasGrownComponentModal: React.Dispatch<React.SetStateAction<boolean>>;      
...
const HowCharacterHasGrownComponent: React.FC<HowCharacterHasGrownComponentProps> = ({
    openHowCharacterHasGrownComponentModal,
    setOpenHowCharacterHasGrownComponentModal,
    selectedCharacter,
    initialStory,
    saveStory,
    refetch,
    ...
    const [howCharacterHasGrown, setHowCharacterHasGrown] = useState<Option[]>([]);
    const [defaultOptions, setDefaultOptions] = useState<Option[]>([]);

    useEffect(() => {
        setHowCharacterHasGrown(selectedCharacter?.howCharacterHasGrown ? selectedCharacter?.howCharacterHasGrown?.map(item => ( { label: item, value: item } )) : []);
        setDefaultOptions(selectedCharacter?.howCharacterHasGrownSuggestions ? selectedCharacter?.howCharacterHasGrownSuggestions?.map(suggestion => ( { label: suggestion, value: suggestion } )) : []);
    }, [selectedCharacter]);

    const getHowCharactersGoalsAndPrioritiesChangedSuggestions = async (payload: string[]) => {
        try {
    ...
    const save = async () => {            
        try {            
            if (howCharacterHasGrown.length < 1) {
                toast.error("Kindly share how the character overcomes obstacles");
                return;
            }
            if (!selectedCharacter?.id) return; 
        
            let howCharacterHasGrownPayload: string[] = howCharacterHasGrown.map(trigger => trigger.value);            
        
            showPageLoader();
            
            const response = await getHowCharactersGoalsAndPrioritiesChangedSuggestions(howCharacterHasGrownPayload);
            if (!response) {
            ...
            let characterUpdated = await updateCharacter({
                howCharacterHasGrown: howCharacterHasGrownPayload,
                storyId: initialStory?.id,
                resolutionStep: 2,      
                howCharactersGoalsAndPrioritiesChangedSuggestions: response?.suggestions,
                suggestedCharacters: response?.suggestedCharacters
            }, selectedCharacter?.id);
        
            refetch();    
            console.log({characterUpdated});
            setOpenHowCharacterHasGrownComponentModal(false);
    ...
    return (
        <>
            <Sheet open={openHowCharacterHasGrownComponentModal} onOpenChange={setOpenHowCharacterHasGrownComponentModal}>
                <SheetContent className="bg-[#cfead1] border border-[#cfead1] overflow-y-scroll sm:min-w-70%] md:min-w-[96%] lg:min-w-[60%] xl:min-w-[50%]">
                    <SheetHeader className=''>
                        <SheetTitle className='font-bold text-lg mt-4'>
                    ...
                    <div className='mt-7'>
                        ...
                        <div className='mt-10'>
                            ...
                            <MultipleSelector
                                creatable
                                value={howCharacterHasGrown}
                                onChange={setHowCharacterHasGrown}
                        ...
                        <ul className='mt-5'>
                        {
                            howCharacterHasGrown?.map((item, index) => (
...
export default HowCharacterHasGrownComponent
